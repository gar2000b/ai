...
/**
 * AlgoV23: minimal variation from AlgoV16 with NO new parameters.
 * Small change: apply an additional fee penalty on each SELL only (fixed extra multiplier).
 * This makes the GA prefer lower-churn solutions. No new params.
 */
public class AlgoV23 {
    ...
    public static double calculateProfit(int fromPos, int toPos, TokenData tokenData) {
        // Small change: extra fee penalty on SELLs only.
        final double EXTRA_SELL_FEE_MULT = 0.50; // adds 50% of normal fee on sell
        ...
        for (int i = fromPos + 1; i <= toPos; i++) {
            ...
            if (order[i] == BUY && !inPos) {
                ...
                double baseFee = FEE_RATE * currentHoldings;
                double extraFee = EXTRA_SELL_FEE_MULT * baseFee;
                currentHoldings -= (baseFee + extraFee);
