...
/**
 * AlgoV4: very simple 2-parameter mean-reversion strategy.
 *
 * Strategy: Z-score of price vs SMA.
 *   sma = SMA(lookbackPeriod)
 *   z   = (close - sma) / stdDev(lookbackPeriod)
 *   BUY  when z-score crosses below -zThreshold
 *   SELL when z-score crosses above +zThreshold
 *
 * Searchable params:
 *  - lookbackPeriod (int)
 *  - zThreshold (double)
...
public class AlgoV4 {
    ...
    static int lookbackPeriod; // rolling window for mean/std
    static double zThreshold;  // z-score threshold
    ...
    /**
     * rawZ(0..511)/INDICATOR_DIVIDER => zThreshold in units.
     */
    ...
    static BitPositions bitPositions = BitPositions.assemble(
            // 7 bits -> 0..127 (clamp to >=5)
            "LOOKBACK_PERIOD", 7,
            // 9 bits -> 0..511 => 0.00..5.11 z
            "Z_THRESHOLD", 9
    );
...
//        WORKING_DIRECTORY = "C:\\work\\discovery-scripts\\data\\pdma\\kraken";
        ...
        int rawZ = bitsToInt(bestSolution,
                bitPositions.get("Z_THRESHOLD").getStartPos(),
                bitPositions.get("Z_THRESHOLD").getEndPos());
        zThreshold = rawZ / (double) INDICATOR_DIVIDER;

        System.out.printf("Best solution (lookbackPeriod, zThreshold): %d, %.2f%n", lookbackPeriod, zThreshold);
...
//        WORKING_DIRECTORY = "C:\\work\\discovery-scripts\\data\\pdma\\kraken";
    ...
    public static double executeStandardTradeRules(String type) {
        ...
        TokenData tokenData = generateCustomSignals(lookbackPeriod, zThreshold);
    ...
    public static TokenData generateCustomSignals(int lookbackPeriod, double zThreshold) {
        ...
        // Compute rolling SMA and std using a simple window (O(n*lookback) but OK for PoC)
        int start = Math.max(lookbackPeriod, 2);
        for (int i = start; i < n; i++) {
            int windowStart = i - lookbackPeriod;
            double sum = 0.0;
            for (int j = windowStart; j < i; j++) sum += close[j];
            ...
            double std = Math.sqrt(var / lookbackPeriod);
            if (std > 0.0) {
        ...
        for (int i = start + 1; i < n; i++) {
            ...
            boolean buyCross = prev >= -zThreshold && cur < -zThreshold;
            boolean sellCross = prev <= zThreshold && cur > zThreshold;
    ...
    private static List<BitString> generateSeedCandidates() {
        ...
        int rawZ = (int) Math.round(zThreshold * INDICATOR_DIVIDER);
        intToBits(seed,
                bitPositions.get("Z_THRESHOLD").getStartPos(),
                bitPositions.get("Z_THRESHOLD").getEndPos(),
                rawZ);
        ...
        System.out.printf("Input zThreshold = %.3f, raw = %d, decoded = %.3f%n",
                zThreshold, rawZ, rawZ / (double) INDICATOR_DIVIDER);
    ...
    private static class FitnessFunction implements FitnessEvaluator<BitString> {
        ...
        public double getFitness(BitString candidate, List<? extends BitString> population) {
            ...
            int rawZ = bitsToInt(candidate,
                    bitPositions.get("Z_THRESHOLD").getStartPos(),
                    bitPositions.get("Z_THRESHOLD").getEndPos());
            double zThreshold = rawZ / (double) INDICATOR_DIVIDER;

            // Guards: need enough data + non-trivial window, and a meaningful threshold.
            if (lookback < 5 || lookback >= ORIGINAL_DATA.getClose().length || zThreshold <= 0.0) {
                return 0;
            }

            TokenData tokenData = generateCustomSignals(lookback, zThreshold);
    ...
    private static class ProgressListener implements EvolutionObserver<BitString> {
        ...
        public void populationUpdate(PopulationData<? extends BitString> data) {
            ...
            int rawZ = bitsToInt(bestCandidate,
                    bitPositions.get("Z_THRESHOLD").getStartPos(),
                    bitPositions.get("Z_THRESHOLD").getEndPos());
            double zThreshold = rawZ / (double) INDICATOR_DIVIDER;
            ...
            System.out.printf(
                    "Generation %d: Best fitness: %.3f, Solution (lookback, zThreshold): %d, %.2f - %s%n",
