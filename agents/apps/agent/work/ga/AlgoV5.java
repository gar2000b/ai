...
/**
 * AlgoV5: simple 2-parameter volatility breakout strategy.
 *
 * Strategy:
 *   - Compute returns r[i] = close[i]/close[i-1] - 1
 *   - Compute rolling std dev of returns over volLookback.
 *   - Compute rolling high over priceLookback.
 *   - BUY  when close crosses above rollingHigh * (1 + volMultiplier * volStd)
 *   - SELL when close crosses below SMA(priceLookback) (simple exit)
 *
 * Searchable params:
 *   - priceLookback (int)
...
public class AlgoV5 {
    ...
    // Fixed/heuristic choices for simplicity:
    static final int VOL_LOOKBACK = 30; // keep volatility estimate stable and not searched
    // End Agentic Workflow Section - 0. fixed variables

    // Begin Agentic Workflow Section - 1. input parameters
    static int priceLookback;      // rolling price high + exit SMA window
    static double volMultiplier;   // scales vol breakout distance
    ...
    /**
     * rawMultiplier(0..1023)/INDICATOR_DIVIDER => volMultiplier in units.
     */
...
//        WORKING_DIRECTORY = "C:\\work\\discovery-scripts\\data\\pdma\\kraken";
        ...
        priceLookback = bitsToInt(bestSolution,
                bitPositions.get("PRICE_LOOKBACK").getStartPos(),
                bitPositions.get("PRICE_LOOKBACK").getEndPos());

        int rawMult = bitsToInt(bestSolution,
                bitPositions.get("VOL_MULTIPLIER").getStartPos(),
                bitPositions.get("VOL_MULTIPLIER").getEndPos());
        volMultiplier = rawMult / (double) INDICATOR_DIVIDER;

        System.out.printf("Best solution (priceLookback, volMultiplier): %d, %.2f%n", priceLookback, volMultiplier);
        System.out.printf("GA execution time: %d milliseconds%n", (endTime - startTime));

        double profit = executeStandardTradeRules("GA");
        int fromPos = Math.max(priceLookback, VOL_LOOKBACK);
...
//        WORKING_DIRECTORY = "C:\\work\\discovery-scripts\\data\\pdma\\kraken";
    ...
    public static double executeStandardTradeRules(String type) {
        ...
        TokenData tokenData = generateCustomSignals(priceLookback, volMultiplier);
    ...
    public static TokenData generateCustomSignals(int priceLookback, double volMultiplier) {
        ...
        // rolling vol std of returns (O(n*VOL_LOOKBACK) for PoC)
        for (int i = VOL_LOOKBACK; i < n; i++) {
            int ws = i - VOL_LOOKBACK;
            double mean = 0.0;
            for (int j = ws; j < i; j++) mean += returns[j];
            mean /= VOL_LOOKBACK;
            double var = 0.0;
            for (int j = ws; j < i; j++) {
            ...
            volStd[i] = Math.sqrt(var / VOL_LOOKBACK);
        }

        // rolling high + SMA(priceLookback)
        double rollingSum = 0.0;
        for (int i = 0; i < n; i++) {
            rollingSum += close[i];
            if (i >= priceLookback) rollingSum -= close[i - priceLookback];
            if (i >= priceLookback - 1) sma[i] = rollingSum / priceLookback;

            if (i >= priceLookback) {
                double high = Double.NEGATIVE_INFINITY;
                for (int j = i - priceLookback; j < i; j++) {
        ...
        int start = Math.max(Math.max(priceLookback, VOL_LOOKBACK), 2);
        for (int i = start; i < n; i++) {
            if (rollingHigh[i] == 0.0 || sma[i] == 0.0) continue;

            double breakoutLevel = rollingHigh[i] * (1.0 + volMultiplier * volStd[i]);

            double prev = close[i - 1];
            double cur = close[i];

            boolean buyCross = prev <= breakoutLevel && cur > breakoutLevel;
            boolean sellCross = prev >= sma[i] && cur < sma[i];

            if (buyCross) orderSignal[i] = BUY;
            else if (sellCross) orderSignal[i] = SELL;
        }

        tokenData.getData().put("Returns", returns);
        tokenData.getData().put("VolStd", volStd);
    ...
    private static List<BitString> generateSeedCandidates() {
        ...
        int rawMult = (int) Math.round(volMultiplier * INDICATOR_DIVIDER);
        intToBits(seed,
                bitPositions.get("VOL_MULTIPLIER").getStartPos(),
                bitPositions.get("VOL_MULTIPLIER").getEndPos(),
                rawMult);
        ...
        System.out.printf("Input priceLookback = %d%n", priceLookback);
        System.out.printf("Input volMultiplier = %.3f, raw = %d, decoded = %.3f%n",
                volMultiplier, rawMult, rawMult / (double) INDICATOR_DIVIDER);
    ...
    private static class FitnessFunction implements FitnessEvaluator<BitString> {
        ...
        public double getFitness(BitString candidate, List<? extends BitString> population) {
            int priceLb = bitsToInt(candidate,
                    bitPositions.get("PRICE_LOOKBACK").getStartPos(),
                    bitPositions.get("PRICE_LOOKBACK").getEndPos());

            int rawMult = bitsToInt(candidate,
                    bitPositions.get("VOL_MULTIPLIER").getStartPos(),
                    bitPositions.get("VOL_MULTIPLIER").getEndPos());
            double mult = rawMult / (double) INDICATOR_DIVIDER;

            int minStart = Math.max(priceLb, VOL_LOOKBACK);

            // Guards
            if (priceLb < 2 || minStart >= ORIGINAL_DATA.getClose().length || mult <= 0.0) {
                return 0;
            }

            TokenData tokenData = generateCustomSignals(priceLb, mult);
            int fromPos = minStart;
            int toPos = ORIGINAL_DATA.getClose().length - 1;
    ...
    private static class ProgressListener implements EvolutionObserver<BitString> {
        ...
        public void populationUpdate(PopulationData<? extends BitString> data) {
            ...
            int priceLb = bitsToInt(bestCandidate,
                    bitPositions.get("PRICE_LOOKBACK").getStartPos(),
                    bitPositions.get("PRICE_LOOKBACK").getEndPos());

            int rawMult = bitsToInt(bestCandidate,
                    bitPositions.get("VOL_MULTIPLIER").getStartPos(),
                    bitPositions.get("VOL_MULTIPLIER").getEndPos());
            double mult = rawMult / (double) INDICATOR_DIVIDER;
            ...
            System.out.printf(
                    "Generation %d: Best fitness: %.3f, Solution (priceLookback, volMultiplier): %d, %.2f - %s%n",
