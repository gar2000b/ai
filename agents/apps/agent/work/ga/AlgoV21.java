...
/**
 * AlgoV21: minimal variation from AlgoV16 with NO new parameters.
 * Small change: change the extreme undervaluation buy trigger from (close/base < 0.69) to (close/base < 0.72).
 * Everything else is unchanged.
 */
public class AlgoV21 {
    ...
    private static void geneticAlgorithmSearch() throws IOException {
        ...
        System.out.printf("Best solution: %.3f, %.3f, %d, %d, %d%n", avgLossMin, avgGainMin, period1, period2, period3);
        System.out.printf("GA execution time: %d ms%n", (endTime - startTime));
    ...
    public static TokenData updateOrderSignal(double avgLossMin, double avgGainMin, int period, double[] closePrices, TokenData tokenData, double stopLoss) {
        ...
        for (int i = period; i < n; i++) {
            ...
            boolean buyCondition = (f < avgLossMin && s < f) || (base != 0.0 && close / base < 0.72); // <-- small change
