...
/**
 * AlgoV25: minimal variation from AlgoV16 with NO new parameters.
 * Small change: penalize deep drawdowns by applying a fixed drawdown penalty factor to fitness.
 * No new params: penalty is computed from max drawdown during backtest.
 */
public class AlgoV25 {
    ...
    public static TokenData updateOrderSignal(double avgLossMin, double avgGainMin, int period, double[] closePrices, TokenData tokenData, double stopLoss) {
        ...
        for (int i = period; i < n; i++) {
            ...
            boolean sellCondition = f < avgGainMin && s > f;
    ...
    public static double calculateProfit(int fromPos, int toPos, TokenData tokenData) {
        // Also compute max drawdown of mark-to-market equity curve.
        double currentHoldings = INITIAL_INVESTMENT;
        boolean inPos = false;
        double tokenAmt = 0.0;

        double peakEquity = INITIAL_INVESTMENT;
        double maxDrawdown = 0.0; // as fraction, e.g. 0.2 for -20%
        ...
        for (int i = fromPos + 1; i <= toPos; i++) {
            ...
            double equity = inPos ? (tokenAmt * close[i]) : currentHoldings;
        ...
        tokenData.getData().put("Max Drawdown", new double[]{maxDrawdown});
    ...
    private static class FitnessFunction implements FitnessEvaluator<BitString> {
        ...
        public double getFitness(BitString candidate, List<? extends BitString> population) {
            ...
            // Small change: apply drawdown penalty.
            double maxDd = tokenData.getData().get("Max Drawdown")[0];
            double penalty = Math.max(0.0, 1.0 - maxDd); // e.g. 0.8 if maxDd=0.2

            double score = (FITNESS_METHOD == FitnessMethod.PROFIT)
                    ? (profit * penalty)
                    : ((profit / hodlProfit) * penalty);
