...
/**
 * AlgoV19: minimal variation from AlgoV16 with NO new parameters.
 * Small change: keep SMA, but smooth the Fast/Slow Avg Gain/Loss using a 3-point EMA.
 * No new parameters: smoothing span is fixed at 3.
 */
public class AlgoV19 {
...
//        WORKING_DIRECTORY = "C:\\work\\discovery-scripts\\data\\pdma\\kraken";
    ...
    public static TokenData generateCustomSignals(double avgLossMin, double avgGainMin, int period1, int period2, int period3) {
        ...
        // Small change: smooth fast/slow vectors with a fixed 3-span EMA.
        tokenData = smoothVectorEma3(tokenData, "Fast Avg Gain/Loss", "Fast Avg Gain/Loss Smoothed");
        tokenData = smoothVectorEma3(tokenData, "Slow Avg Gain/Loss", "Slow Avg Gain/Loss Smoothed");
    ...
    /**
     * Fixed 3-point EMA smoothing (no new params).
     */
    public static TokenData smoothVectorEma3(TokenData tokenData, String inputKey, String outputKey) {
        double[] in = tokenData.getData().get(inputKey);
        double[] out = new double[in.length];

        if (in.length == 0) {
            tokenData.getData().put(outputKey, out);
            return tokenData;
        }
    ...
    public static TokenData updateOrderSignal(double avgLossMin, double avgGainMin, int period, double[] closePrices, TokenData tokenData, double stopLoss) {
        ...
        // Small change: use smoothed vectors.
        double[] fastAvgGainLoss = tokenData.getData().get("Fast Avg Gain/Loss Smoothed");
        double[] slowAvgGainLoss = tokenData.getData().get("Slow Avg Gain/Loss Smoothed");
    ...
    private static class ProgressListener2 implements EvolutionObserver<BitString> {
        private BitString bestCandidate;
