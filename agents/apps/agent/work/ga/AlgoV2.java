...
/**
 * AlgoV2: a very simple 2-parameter strategy variant.
 *
 * Strategy: SMA cross with a band.
 *  - Compute SMA(lookbackPeriod)
 *  - BUY  when close crosses above SMA * (1 + bandPct)
 *  - SELL when close crosses below SMA * (1 - bandPct)
 *
 * Searchable params:
 *  - lookbackPeriod (int)
 *  - bandPct (double, %) (0..10.23% via encoding)
 */
public class AlgoV2 {
    /**
    ...
    static int lookbackPeriod; // SMA window
    static double bandPct;     // percent band around SMA
    ...
    /**
     * Encoding divider.
     * rawBand(0..1023)/INDICATOR_DIVIDER => bandPct in percent.
     */
...
//        WORKING_DIRECTORY = "C:\\work\\discovery-scripts\\data\\pdma\\kraken";
        ...
        int rawBand = bitsToInt(bestSolution,
                bitPositions.get("BAND_PCT").getStartPos(),
                bitPositions.get("BAND_PCT").getEndPos());
        bandPct = rawBand / (double) INDICATOR_DIVIDER;

        System.out.printf("Best solution (lookbackPeriod, bandPct%%): %d, %.2f%%%n", lookbackPeriod, bandPct);
...
//        WORKING_DIRECTORY = "C:\\work\\discovery-scripts\\data\\pdma\\kraken";
    ...
    public static double executeStandardTradeRules(String type) {
        ...
        TokenData tokenData = generateCustomSignals(lookbackPeriod, bandPct);
    ...
    public static TokenData generateCustomSignals(int lookbackPeriod, double bandPct) {
        ...
        for (int i = 0; i < n; i++) {
            rollingSum += close[i];
            if (i >= lookbackPeriod) {
                rollingSum -= close[i - lookbackPeriod];
            }
            if (i >= lookbackPeriod - 1) {
                sma[i] = rollingSum / lookbackPeriod;
            }
        }

        double band = bandPct / 100.0;
        int start = Math.max(lookbackPeriod, 1);

        for (int i = start; i < n; i++) {
            if (sma[i] == 0.0) continue;

            double buyLevel = sma[i] * (1.0 + band);
            double sellLevel = sma[i] * (1.0 - band);
    ...
    private static List<BitString> generateSeedCandidates() {
        ...
        int rawBand = (int) Math.round(bandPct * INDICATOR_DIVIDER);
        intToBits(seed,
                bitPositions.get("BAND_PCT").getStartPos(),
                bitPositions.get("BAND_PCT").getEndPos(),
                rawBand);
        ...
        System.out.printf("Input bandPct = %.3f%%, raw = %d, decoded = %.3f%%%n",
                bandPct, rawBand, rawBand / (double) INDICATOR_DIVIDER);
    ...
    private static class FitnessFunction implements FitnessEvaluator<BitString> {
        ...
        public double getFitness(BitString candidate, List<? extends BitString> population) {
            ...
            int rawBand = bitsToInt(candidate,
                    bitPositions.get("BAND_PCT").getStartPos(),
                    bitPositions.get("BAND_PCT").getEndPos());
            double bandPct = rawBand / (double) INDICATOR_DIVIDER;
            ...
            TokenData tokenData = generateCustomSignals(lookback, bandPct);
    ...
    private static class ProgressListener implements EvolutionObserver<BitString> {
        ...
        public void populationUpdate(PopulationData<? extends BitString> data) {
            ...
            int rawBand = bitsToInt(bestCandidate,
                    bitPositions.get("BAND_PCT").getStartPos(),
                    bitPositions.get("BAND_PCT").getEndPos());
            double bandPct = rawBand / (double) INDICATOR_DIVIDER;
            ...
            System.out.printf(
                    "Generation %d: Best fitness: %.3f, Solution (lookback, bandPct%%): %d, %.2f%% - %s%n",
