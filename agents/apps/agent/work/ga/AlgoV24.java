...
/**
 * AlgoV24: minimal variation from AlgoV16 with NO new parameters.
 * Small change: require confirmation for BUY and SELL by needing the condition to hold
 * for 2 consecutive bars (fixed). No new params.
 */
public class AlgoV24 {
    ...
    /**
     * Small change: require 2-bar confirmation for buy and sell.
     */
    public static TokenData updateOrderSignal(double avgLossMin, double avgGainMin, int period, double[] closePrices, TokenData tokenData, double stopLoss) {
        ...
        for (int i = period; i < n; i++) {
            ...
            boolean buyNow = (f < avgLossMin && s < f) || (base != 0.0 && close / base < 0.69);
            boolean sellNow = (f < avgGainMin && s > f);

            boolean buyPrev = (i > 0) && (((fast[i - 1] < avgLossMin && slow[i - 1] < fast[i - 1]) || (sma[i - 1] != 0.0 && closePrices[i - 1] / sma[i - 1] < 0.69)));
            boolean sellPrev = (i > 0) && ((fast[i - 1] < avgGainMin && slow[i - 1] > fast[i - 1]));

            boolean buyCondition = buyNow && buyPrev;
            boolean sellCondition = sellNow && sellPrev;
