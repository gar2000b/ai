...
// Final-layer loss-protection (curve flattening)
// Use a trailing stop once the position is in profit; sell if price falls from peak by this fraction.
static double TRAILING_STOP_PCT;

// Optional cooldown after a forced protective sell to avoid immediate re-entry.
// Measured in bars/candles at DATA_RESOLUTION.
static int PROTECTIVE_COOLDOWN_BARS;
...
// Final-layer GA-searchable loss-protection knobs
// - ATR (avg true range) period for volatility estimation
// - Multiple of ATR that triggers an emergency exit on a sudden adverse move
static int atrPeriod;
static double atrStopMultiplier;
...
static BitPositions bitPositions = BitPositions.assemble(
        // indicator thresholds (scaled by INDICATOR_DIVIDER)
        "AVG_LOSS_MIN", 6,
        "AVG_GAIN_MIN", 6,
        ...
        // final-layer loss protection
        // ATR period: 1..255 is sufficient and keeps search space reasonable
        "ATR_PERIOD", 8,
        // ATR stop multiplier: signed to allow exploration around 0..~3.0 when scaled by INDICATOR_DIVIDER
        "ATR_STOP_MULT", 10
);
...
//            WORKING_DIRECTORY = "C:\\work\\discovery-scripts\\data\\pdma\\kraken";
    ...
    // Final-layer loss protection search params
    atrPeriod = 56;
    atrStopMultiplier = 1.600; // e.g. sell if a sudden drop exceeds 1.6x ATR from purchase
    ...
    // Final-layer hard-fixed params
    TRAILING_STOP_PCT = 0.090;         // 9% trail from peak while in-position
    PROTECTIVE_COOLDOWN_BARS = 8;      // wait 8 bars after a forced protective sell
...
//            linearScale = generateLinearScale(1, 10, ORIGINAL_DATA.getClose().length);
    ...
    // Decode final-layer protection params
    atrPeriod = bitsToInt(bestSolution, bitPositions.get("ATR_PERIOD").getStartPos(), bitPositions.get("ATR_PERIOD").getEndPos());

    int rawAtrMult = bitsToInt(bestSolution, bitPositions.get("ATR_STOP_MULT").getStartPos(), bitPositions.get("ATR_STOP_MULT").getEndPos());
    int atrMultBits = bitPositions.get("ATR_STOP_MULT").getEndPos() - bitPositions.get("ATR_STOP_MULT").getStartPos();
    double atrMultMid = Math.pow(2, atrMultBits) / 2.0;
    atrStopMultiplier = (rawAtrMult - atrMultMid) / INDICATOR_DIVIDER;

    System.out.printf(
            "Best solution (avgLossMin, avgGainMin, period1, period2, period3, atrPeriod, atrStopMultiplier): %.3f, %.3f, %d, %d, %d, %d, %.3f%n",
            avgLossMin,
            avgGainMin,
            period1,
            period2,
...
//            WORKING_DIRECTORY = "C:\\work\\discovery-scripts\\data\\pdma\\kraken";
    ...
    // Final-layer loss protection params
    atrPeriod = 56;
    atrStopMultiplier = 1.600;
    ...
    // Final-layer hard-fixed params
    TRAILING_STOP_PCT = 0.090;
    PROTECTIVE_COOLDOWN_BARS = 8;
...
public static double executeStandardTradeRules(String type) {
    ...
    TokenData tokenData = generateCustomSignals(avgLossMin, avgGainMin, period1, period2, period3, atrPeriod, atrStopMultiplier);
...
public static TokenData generateCustomSignals(double avgLossMin,
                                              double avgGainMin,
                                              int period1,
                                              int period2,
                                              int period3,
    ...
    // Layer 4 begin.
    // Final-layer sudden-loss protection:
    // 1) compute ATR (volatility proxy)
    // 2) if in-position, force SELL when a sudden adverse move exceeds ATR multiple
    // 3) add a trailing-stop SELL from post-entry peak to flatten drawdowns
    // 4) optional cooldown to prevent immediate re-entry after a forced protective sell
    tokenData = updateAtr(atrPeriod, ORIGINAL_DATA, tokenData);
    tokenData = applyFinalLossProtectionLayer(
            period1 + period3 - 1,
            ORIGINAL_DATA.getClose(),
            tokenData,
            atrStopMultiplier,
            TRAILING_STOP_PCT,
            PROTECTIVE_COOLDOWN_BARS
    );
    // Layer 4 end.
...
/**
 * Average True Range (Wilder style) computed as an SMA of True Range.
 * Uses ORIGINAL_DATA high/low/close.
 */
public static TokenData updateAtr(int atrPeriod, TokenData originalData, TokenData tokenData) {
    int n = originalData.getClose().length;
    double[] atr = new double[n];
    Arrays.fill(atr, 0.0);

    if (atrPeriod < 2) {
        tokenData.getData().put("ATR", atr);
        return tokenData;
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (i < atrPeriod) {
            ...
            atr[i] = rolling / atrPeriod;
            rolling -= tr[i - atrPeriod + 1];
        }
    }
...
public static TokenData applyFinalLossProtectionLayer(int startPos,
                                                     double[] closePrices,
                                                     TokenData tokenData,
                                                     double atrStopMultiplier,
    ...
    double[] atr = tokenData.getData().get("ATR");

    boolean inPosition = false;
    double entryPrice = 0.0;
    double peakSinceEntry = 0.0;
    int cooldown = 0;

    for (int i = startPos; i < n; i++) {
        // enforce cooldown by cancelling BUYs
        if (cooldown > 0) {
            if (orderSignal[i] == BUY) {
                orderSignal[i] = 0.0; // HOLD
            }
            cooldown--;
        }

        // state transitions on existing signals
        ...
        if (close > peakSinceEntry) {
            peakSinceEntry = close;
        }

        // Only act if ATR is usable.
        double currentAtr = (atr != null && i < atr.length) ? atr[i] : 0.0;

        // (A) Sudden-loss / shock-stop: if price drops from entry by atrStopMultiplier * ATR
        // Guard: ignore non-sensical multipliers.
        if (currentAtr > 0.0 && atrStopMultiplier > 0.0) {
            double shockStop = entryPrice - (atrStopMultiplier * currentAtr);
            if (close <= shockStop) {
                orderSignal[i] = SELL;
                inPosition = false;
                entryPrice = 0.0;
        ...
        // (B) Trailing stop from peak to protect profits / flatten drawdowns.
        if (trailingStopPct > 0.0) {
            double trailStop = peakSinceEntry * (1.0 - trailingStopPct);
            if (close <= trailStop) {
                orderSignal[i] = SELL;
                inPosition = false;
                entryPrice = 0.0;
...
private static List<BitString> generateSeedCandidates() {
    ...
    // Final-layer params
    intToBits(seed, bitPositions.get("ATR_PERIOD").getStartPos(), bitPositions.get("ATR_PERIOD").getEndPos(), atrPeriod);

    int atrMultBits = bitPositions.get("ATR_STOP_MULT").getEndPos() - bitPositions.get("ATR_STOP_MULT").getStartPos();
    double atrMultMid = Math.pow(2, atrMultBits) / 2.0;
    int rawAtrMult = (int) Math.round((atrStopMultiplier * INDICATOR_DIVIDER) + atrMultMid);
    intToBits(seed, bitPositions.get("ATR_STOP_MULT").getStartPos(), bitPositions.get("ATR_STOP_MULT").getEndPos(), rawAtrMult);
    ...
    System.out.printf("ATR Period: %d%n", atrPeriod);
    System.out.printf("ATR Stop Mult: %.6f, raw = %d, decoded = %.6f%n",
            atrStopMultiplier, rawAtrMult, (rawAtrMult - atrMultMid) / INDICATOR_DIVIDER);
...
private static class FitnessFunction implements FitnessEvaluator<BitString> {
    ...
    public double getFitness(BitString candidate, List<? extends BitString> population) {
        ...
        int atrPeriod = bitsToInt(candidate, bitPositions.get("ATR_PERIOD").getStartPos(), bitPositions.get("ATR_PERIOD").getEndPos());

        int rawAtrMult = bitsToInt(candidate, bitPositions.get("ATR_STOP_MULT").getStartPos(), bitPositions.get("ATR_STOP_MULT").getEndPos());
        int atrMultBits = bitPositions.get("ATR_STOP_MULT").getEndPos() - bitPositions.get("ATR_STOP_MULT").getStartPos();
        double atrMultMid = Math.pow(2, atrMultBits) / 2.0;
        double atrStopMultiplier = (rawAtrMult - atrMultMid) / INDICATOR_DIVIDER;

        // Guards: restrict to sensible search space (avoid invalid/degenerate solutions).
        if (period1 < 2 || period2 < 2 || period3 < 2) {
            return 0;
        }
        if (period2 > period3) {
            return 0;
        }
        if ((period1 + period3) > ORIGINAL_DATA.getClose().length) {
            return 0;
        }
        // ATR: needs enough lookback; also avoid huge periods.
        if (atrPeriod < 2 || atrPeriod > 255) {
            return 0;
        }
        // The multiplier is only meaningful when positive. Negative/zero is redundant (never triggers), so penalise.
...
private static class ProgressListener implements EvolutionObserver<BitString> {
    ...
    public void populationUpdate(PopulationData<? extends BitString> data) {
        ...
        int atrPeriod = bitsToInt(bestCandidate, bitPositions.get("ATR_PERIOD").getStartPos(), bitPositions.get("ATR_PERIOD").getEndPos());

        int rawAtrMult = bitsToInt(bestCandidate, bitPositions.get("ATR_STOP_MULT").getStartPos(), bitPositions.get("ATR_STOP_MULT").getEndPos());
        int atrMultBits = bitPositions.get("ATR_STOP_MULT").getEndPos() - bitPositions.get("ATR_STOP_MULT").getStartPos();
        double atrMultMid = Math.pow(2, atrMultBits) / 2.0;
        double atrStopMultiplier = (rawAtrMult - atrMultMid) / INDICATOR_DIVIDER;
        ...
        System.out.printf(
                "Generation %d: Best fitness: %.3f, Solution (avgLossMin, avgGainMin, period1, period2, period3, atrPeriod, atrStopMultiplier): %.3f, %.3f, %d, %d, %d, %d, %.3f - %s%n",
