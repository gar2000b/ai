...
/**
 * AlgoV14 (x4 params): VWAP reversion with volatility and time stop.
 *
 * Notes:
 *  - TokenData may not include volume in this project; so we implement a proxy VWAP
 *    using (high+low+close)/3 as typical price and treat each bar equally (i.e. TWAP).
 *  - This is still useful as a conceptual baseline.
 *
 * Strategy:
 *  - proxyVWAP = SMA(typicalPrice, vwapPeriod)
 *  - deviationPct = 100*(close/proxyVWAP - 1)
 *  - BUY when deviationPct crosses below -entryDevPct AND volStdPct <= maxVolPct
 *  - SELL when deviationPct crosses above -exitDevPct OR maxHoldBars reached
 *
 * Searchable params:
 *  - vwapPeriod (int)
 *  - entryDevPct (double %)
...
public class AlgoV14 {
    ...
    static final int VOL_LOOKBACK = 30;
    // End Agentic Workflow Section - 0. fixed variables

    // Begin Agentic Workflow Section - 1. input parameters
    static int vwapPeriod;
    static double entryDevPct;
    static double exitDevPct;
    static int maxHoldBars;
    ...
    private static void geneticAlgorithmSearch() throws IOException {
        ...
        vwapPeriod = bitsToInt(best, bitPositions.get("VWAP_PERIOD").getStartPos(), bitPositions.get("VWAP_PERIOD").getEndPos());
        entryDevPct = bitsToInt(best, bitPositions.get("ENTRY_DEV").getStartPos(), bitPositions.get("ENTRY_DEV").getEndPos()) / (double) INDICATOR_DIVIDER;
        exitDevPct = bitsToInt(best, bitPositions.get("EXIT_DEV").getStartPos(), bitPositions.get("EXIT_DEV").getEndPos()) / (double) INDICATOR_DIVIDER;
        maxHoldBars = bitsToInt(best, bitPositions.get("MAX_HOLD").getStartPos(), bitPositions.get("MAX_HOLD").getEndPos());

        System.out.printf("Best (vwapP, entry%%, exit%%, maxHold): %d, %.2f%%, %.2f%%, %d%n",
                vwapPeriod, entryDevPct, exitDevPct, maxHoldBars);

        double profit = executeStandardTradeRules("GA");
        int fromPos = Math.max(vwapPeriod, VOL_LOOKBACK) + 2;
    ...
    public static double executeStandardTradeRules(String type) {
        TokenData td = generateCustomSignals(vwapPeriod, entryDevPct, exitDevPct, maxHoldBars);
        int fromPos = Math.max(vwapPeriod, VOL_LOOKBACK) + 2;
        int toPos = ORIGINAL_DATA.getClose().length - 1;
    ...
    public static TokenData generateCustomSignals(int vwapPeriod, double entryDevPct, double exitDevPct, int maxHoldBars) {
        ...
        double[] devPct = new double[n];
        double[] returns = new double[n];
        double[] volStdPct = new double[n];
        double[] orderSignal = new double[n];

        Arrays.fill(typical, 0.0);
        Arrays.fill(vwap, 0.0);
        Arrays.fill(devPct, 0.0);
        ...
        // proxy vwap = SMA(typical)
        double sum = 0.0;
        for (int i = 0; i < n; i++) {
            sum += typical[i];
            if (i >= vwapPeriod) sum -= typical[i - vwapPeriod];
            if (i >= vwapPeriod - 1) vwap[i] = sum / vwapPeriod;
        }
        ...
        for (int i = VOL_LOOKBACK; i < n; i++) {
            double mean = 0.0;
            for (int j = i - VOL_LOOKBACK; j < i; j++) mean += returns[j];
            mean /= VOL_LOOKBACK;
            double var = 0.0;
            for (int j = i - VOL_LOOKBACK; j < i; j++) {
                double d = returns[j] - mean;
                var += d * d;
            }
            volStdPct[i] = Math.sqrt(var / VOL_LOOKBACK) * 100.0;
        }

        int start = Math.max(Math.max(vwapPeriod, VOL_LOOKBACK), 2);
        for (int i = start; i < n; i++) {
            if (vwap[i] != 0.0) devPct[i] = 100.0 * (close[i] / vwap[i] - 1.0);
        }
        ...
        for (int i = start + 1; i < n; i++) {
            if (!inPosition) {
                boolean crossBelow = devPct[i - 1] >= -entryDevPct && devPct[i] < -entryDevPct;
                boolean volOk = volStdPct[i] <= 5.0; // fixed safety cap
                if (crossBelow && volOk) {
                    orderSignal[i] = BUY;
                    inPosition = true;
                    holdBars = 0;
                ...
                holdBars++;
                boolean crossUp = devPct[i - 1] <= -exitDevPct && devPct[i] > -exitDevPct;
                boolean timeStop = holdBars >= maxHoldBars;
                if (crossUp || timeStop) {
                    orderSignal[i] = SELL;
                    inPosition = false;
                    holdBars = 0;
        ...
        td.getData().put("VWAP", vwap);
        td.getData().put("DevPct", devPct);
    ...
    private static List<BitString> generateSeedCandidates() {
        ...
        intToBits(seed, bitPositions.get("VWAP_PERIOD").getStartPos(), bitPositions.get("VWAP_PERIOD").getEndPos(), vwapPeriod);
        intToBits(seed, bitPositions.get("ENTRY_DEV").getStartPos(), bitPositions.get("ENTRY_DEV").getEndPos(), (int) Math.round(entryDevPct * INDICATOR_DIVIDER));
        intToBits(seed, bitPositions.get("EXIT_DEV").getStartPos(), bitPositions.get("EXIT_DEV").getEndPos(), (int) Math.round(exitDevPct * INDICATOR_DIVIDER));
        intToBits(seed, bitPositions.get("MAX_HOLD").getStartPos(), bitPositions.get("MAX_HOLD").getEndPos(), maxHoldBars);
    ...
    private static class FitnessFunction implements FitnessEvaluator<BitString> {
        ...
        public double getFitness(BitString candidate, List<? extends BitString> population) {
            int p = bitsToInt(candidate, bitPositions.get("VWAP_PERIOD").getStartPos(), bitPositions.get("VWAP_PERIOD").getEndPos());
            double entry = bitsToInt(candidate, bitPositions.get("ENTRY_DEV").getStartPos(), bitPositions.get("ENTRY_DEV").getEndPos()) / (double) INDICATOR_DIVIDER;
            double exit = bitsToInt(candidate, bitPositions.get("EXIT_DEV").getStartPos(), bitPositions.get("EXIT_DEV").getEndPos()) / (double) INDICATOR_DIVIDER;
            int maxHold = bitsToInt(candidate, bitPositions.get("MAX_HOLD").getStartPos(), bitPositions.get("MAX_HOLD").getEndPos());

            int fromPos = Math.max(p, VOL_LOOKBACK) + 2;
            if (p < 5 || maxHold < 5 || fromPos >= ORIGINAL_DATA.getClose().length) return 0;
            if (entry <= 0.0 || exit < 0.0 || exit > entry) return 0;

            TokenData td = generateCustomSignals(p, entry, exit, maxHold);
            int toPos = ORIGINAL_DATA.getClose().length - 1;
    ...
    private static class ProgressListener implements EvolutionObserver<BitString> {
        ...
        public void populationUpdate(PopulationData<? extends BitString> data) {
            ...
            int p = bitsToInt(bestCandidate, bitPositions.get("VWAP_PERIOD").getStartPos(), bitPositions.get("VWAP_PERIOD").getEndPos());
            double entry = bitsToInt(bestCandidate, bitPositions.get("ENTRY_DEV").getStartPos(), bitPositions.get("ENTRY_DEV").getEndPos()) / (double) INDICATOR_DIVIDER;
            double exit = bitsToInt(bestCandidate, bitPositions.get("EXIT_DEV").getStartPos(), bitPositions.get("EXIT_DEV").getEndPos()) / (double) INDICATOR_DIVIDER;
            int maxHold = bitsToInt(bestCandidate, bitPositions.get("MAX_HOLD").getStartPos(), bitPositions.get("MAX_HOLD").getEndPos());
            ...
            System.out.printf(
                    "Generation %d: Best fitness: %.3f, Solution (p, entry%%, exit%%, maxHold): %d, %.2f%%, %.2f%%, %d - %s%n",
                    data.getGenerationNumber(),
                    data.getBestCandidateFitness(),
                    p, entry, exit, maxHold,
                    formatted
            );
        }
