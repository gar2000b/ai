...
from commands.command import _run_os_command, clone_java, list_file, run_program
from prompt_toolkit.auto_suggest import AutoSuggestFromHistory
from prompt_toolkit.validation import Validator

from agents.llm import clear_messages, get_context_size, llm, print_messages
    ...
    while True:
        try:
            command = session.prompt(
                [("class:prompt-tokens", f"[Context Window - Tokens: {get_context_size()}]\n"), ("class:prompt", "Prompt: ")],
        ...
        try:
            if command == "help":
                ...
                clear_screen()
            elif command == "clear context" or command == "context":
                clear_messages()
                print(f"\n{ansi.DIM}Chat context cleared.{ansi.RESET}\n")
            elif command == "print messages" or command == "messages" or command == "print message history":
                print_messages()
            elif command == "reset":
                clear_screen()
                print_welcome()
            ...
            elif command == "clone java" or command.startswith("clone java "):
                parts = command.split(maxsplit=3)
                if len(parts) < 3 or not parts[2].strip():
                    print(f"\n{ansi.DIM}Usage: clone java <source.java> <target.java> [path]{ansi.RESET}\n")
                else:
                    ...
                    if len(parts) < 4 or not parts[3].strip():
                        ...
                        path = ""
                        result = clone_java(source_file, target_file, path)
                        if not result.get("success"):
                            print(f"\n{ansi.DIM}Error: {result.get('error', 'Unknown error')}{ansi.RESET}\n")
