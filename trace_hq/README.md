...
- **Keyboard:** From Workflows or Backlog, **C** opens the create-story modal (placement: Backlog or any workflow). With a story card selected, **E** opens the edit-story modal and **D** opens the delete-story confirmation modal (logical delete: story is hidden from the app but remains in the database). On Workflows, **P** opens the create-project modal and **W** opens the create-workflow modal. Whilst any of these modals is open, **C**, **E**, **P**, **W**, and **D** are ignored so you can type those letters in form fields.
...
3. **Apply schema and seed data** (if not already done)
   ...
   From the `trace_hq` directory, apply the DDL in `database/schema/` in order (including migrations such as 02a, 04a, 04c, 04d — see `database/schema/README.md`), then run the scripts in `database/data/` in order (see `markdown/database/DATABASE.md`).
