...
- **Project selector:** The project dropdown is in the **top-right** of the Workflows view (in the board header), so it is easily accessible and does not clutter the side menu. It should look integrated with the board (e.g. same visual language as the rest of the UI). An **Edit** link and a red **Delete** link appear beside the project dropdown when a project is selected. **Edit** opens a simple modal pre-filled with the current project name; the user can change it and save (or cancel). **Delete** opens a confirmation modal and **logically deletes** the current project (hidden from the board; data kept in the database).
...
- **Deleting a workflow:** Each workflow section has a red **Delete** link beside the Edit link. Clicking it opens a confirmation modal (“Are you sure you wish to delete the workflow …?”). The workflow is **logically deleted** (hidden from the board; the row and its stages remain in the database). Stories in that workflow no longer appear on the board until the workflow is restored.
...
- **Keyboard shortcuts and modals:** Whilst any of the create-story (C), edit-story (E), create-project (P), create-workflow (W), edit-workflow, edit-project, delete-story (D), or delete-workflow modals are open, **C**, **E**, **P**, **W**, and **D** key events are ignored so users can type those letters in form fields without triggering the shortcuts.
